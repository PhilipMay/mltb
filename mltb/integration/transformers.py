...
def omlflow_callback_factory(parent):
    """
    Factory function that returns the OMLflowCallback class, inheriting from the parent class.

    Handles the case where at import time the Transformers package may not be available, and a
    MockTrainerCallback class may be substituted for the transformers.TrainerCallback class.

    Args:
        parent: parent class that OMLflowCallback will inherit from

    Returns:
        OMLflowCallback class, inheriting from parent

    """

    class OMLflowCallback(parent):
        """
    ...
    return OMLflowCallback


class MockTrainerCallback:
    """
    Mock Transformers TrainerCallback when Transformers package is not installed.
    """

    pass


# Expose the OMLflowCallback class, inheriting from transformers.TrainerCallback
# if the transformers module is available; otherwise expose a non-functional
# OMLflowCallback class that raises an ImportError on init
try:
    _imports.check()
except ImportError:
    OMLflowCallback = omlflow_callback_factory(MockTrainerCallback)
else:
    OMLflowCallback = omlflow_callback_factory(transformers.TrainerCallback)
